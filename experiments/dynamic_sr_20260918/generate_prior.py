#!/usr/bin/env python3
"""Frozen official SwinIR-M DF2K x4 prior, train-LR inputs only.
...
def load_inventory(manifest_path: Path, cameras: str | None):
    ...
    if len(train) != len(set(train)) or set(train) & forbidden or 'cam00' in train:
...
def main():
    ...
    parser.add_argument('--dependency-path', type=Path, default=SCRIPT_ROOT / 'vendor')
