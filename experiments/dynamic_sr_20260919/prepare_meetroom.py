"""Prepare official MeetRoom LR inputs and training-only SIFT triangulation.

Adapted from 20260918 prepare_n3dv.py; native HR geometry and numeric camera IDs.
...
def parse_meetroom_cameras(raw_dir):
    raw_dir = Path(raw_dir)
    # Official extractor writes cam_10.mp4 -> cam10.png; official LLFF loader
    # then sorts cam00,...cam12. Sorting raw cam_*.mp4 lexically is WRONG.
    videos = sorted(raw_dir.glob("cam_*.mp4"), key=lambda p: int(p.stem.split("_")[-1]))
    bounds = np.load(raw_dir / "poses_bounds.npy", allow_pickle=False)
    assert len(videos) == len(bounds) == 13
    assert [int(p.stem.split("_")[-1]) for p in videos] == list(range(13))
    cameras = {}
    down = np.array([[.25, 0, -.375], [0, .25, -.375], [0, 0, 1]])
    for index, (row, video) in enumerate(zip(bounds, videos)):
        pose = row[:-2].reshape(3, 5)
        height, width, focal = pose[:, 4]
        assert (int(width), int(height)) == (1280, 720)
        c2w = np.eye(4)
        c2w[:3, :3] = np.stack((pose[:, 1], pose[:, 0], -pose[:, 2]), axis=1)
        c2w[:3, 3] = pose[:, 3]
        assert np.allclose(c2w[:3,:3].T @ c2w[:3,:3], np.eye(3), atol=2e-3)
        assert np.isclose(np.linalg.det(c2w[:3,:3]), 1., atol=2e-3)
        k = np.array([[focal, 0, width/2], [0, focal, height/2], [0, 0, 1]])
        camera_id = f"cam{index:02d}"
        cameras[camera_id] = {"camera_id": camera_id, "video": str(video.resolve()),
            "raw_width": int(width), "raw_height": int(height), "pose_row_index": index,
            "K_raw": k.tolist(), "K_hr": k.tolist(), "K_lr": (down @ k).tolist(),
            "c2w": c2w.tolist(), "w2c": np.linalg.inv(c2w).tolist(),
...
def extract_camera(camera: dict, destination: Path, frame_indices: list[int], split: str) -> list[dict]:
    ...
    try:
        for frame_index in range(max(frame_indices) + 1):
            ...
            low_tensor = F.interpolate(image_tensor, size=(180, 320), mode="bicubic",
...
def make_initialization(root: Path, cameras: dict, train_ids: list[str], frame_indices: list[int],
    ...
    for frame in selected_frames:
        ...
        for camera_id in train_ids:
            ...
            if bgr is None or bgr.shape[:2] != (180, 320):
...
def main() -> None:
    ...
    start = time.time()
    cameras = parse_meetroom_cameras(args.raw_dir)
    ...
    manifest = {"schema": "n3dv_dynamic_sr_pilot_v1", "scene": "meetroom_" + args.raw_dir.name,
                "source": "https://github.com/AlgoHunt/StreamRF", "scale": 4,
                "raw_directory": str(args.raw_dir.resolve()),
                "poses_bounds_sha256": sha256(args.raw_dir / "poses_bounds.npy"),
                "dataset_family": "MeetRoom / StreamRF",
                "capture": {"official_sync": "external pulse synchronizes shutters", "fps": 30, "frame_count": 300, "depth_used": False},
                "download_receipts_path": str(args.raw_dir.resolve() / "download_receipts.json"),
                "frame_indices": frame_indices, "time_convention": "original_frame_index / 300.0",
                "resolutions": {"hr": [1280, 720], "lr": [320, 180]},
                "degradation": {"raw_to_reference": "native decoded 1280x720 RGB; no resize or crop",
                ...
                "split_note": "Official StreamRF extractor maps numeric raw cam_N to zero-padded camNN; official LLFF loader with meetroom llffhold=100 tests sorted index0 (cam00). cam01 is additionally held out for development: 11 train +1 dev +1 test, so not the official 12 train camera benchmark.",
                ...
                "script_sha256": {"prepare_meetroom.py": sha256(Path(__file__))}}
