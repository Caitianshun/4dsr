...
def make_protocol(m,prior):
 ...
 for c in prior:
  for f in FRAMES:
   ...
   proposals.append(p)
 return dict(version='candidate2_v2',scales=SCALES,proposals=proposals,
  ...
  design='Fixed pre-result camera/time IDs, all scales reported. Primary scale 0.005. A-only smoke calibration found 0.1 and 0.4 increase the proposing SR loss; those overshoots are retained in the original smoke output. The new common scales use only that numerical calibration, not HR outcome. Secondary scales test local finite-step behavior, not independent samples. No HR used in proposal, observation selection, step normalization, gate thresholds, or rankings.',
...
def summarize(rows,protocol):
 ...
 for scale in SCALES:
  ...
  for target in ['novel','train_cross']:
   ms={}
   for gate in ['near','far','cross','random','A_actual','A_derivative','A_train_L1_actual','A_train_L1_derivative']:
    scores=np.array([r['scores'][gate] for r in rs]);d=np.array([r['eval_delta'][target]['psnr'] for r in rs])
    tol=np.array([r['score_thresholds'][gate] for r in rs])
    accept=scores>tol
...
def run(args):
 ...
 for i,p in enumerate(protocol['proposals']):
  ...
  raw=render_image(g,data.cam(a))['render'];t=data.lr(a);lpred=downsample(raw,t.shape[-2:]);lr_mse=(lpred-t).square().mean();lr_mse.backward(retain_graph=True)
  ...
  g.optimizer.zero_grad(set_to_none=True)
  lr_l1=(lpred-t).abs().mean();lr_l1.backward()
  lr_l1_deriv=sum(float((v.grad.detach().double()*direction[k]).sum()) for k,v in params.items() if v.grad is not None)
  g.optimizer.zero_grad(set_to_none=True);del raw,lr_mse,lpred,lr_l1
  ...
  for epsilon in [.001,.005]:
   ...
   for sign in [-1,1]:
    ...
    lm=measure_lr(g,data,a);observations.append((ts,lm['mse'],lm['l1']))
   fd_sr=(observations[1][0]-observations[0][0])/(2*epsilon);fd_lr=(observations[1][1]-observations[0][1])/(2*epsilon);fd_l1=(observations[1][2]-observations[0][2])/(2*epsilon)
   finite_difference.append(dict(epsilon=epsilon,sr_analytic=sr_derivative,sr_fd=fd_sr,sr_relative_error=abs(fd_sr-sr_derivative)/max(abs(sr_derivative),1e-12),lr_analytic=lr_deriv,lr_fd=fd_lr,lr_relative_error=abs(fd_lr-lr_deriv)/max(abs(lr_deriv),1e-12),lr_l1_analytic=lr_l1_deriv,lr_l1_fd=fd_l1,lr_l1_relative_error=abs(fd_l1-lr_l1_deriv)/max(abs(lr_l1_deriv),1e-12)))
  ...
  for scale in SCALES:
   ...
   scores['A_train_L1_actual']=lrbase[a]['l1']-after[a]['l1'];scores['A_train_L1_derivative']=-scale*lr_l1_deriv
   thresholds={b:(1e-7 if 'train_L1' in b else tol) for b in scores}
   row=dict(proposal_id=p['id'],A=a,B=p['B'],scale=scale,scores=scores,score_threshold=tol,derivative_threshold=tol,score_thresholds=thresholds,
      gate_state={b:('improve' if v>thresholds[b] else 'worsen' if v < -thresholds[b] else 'unresolved') for b,v in scores.items()},
      A_teacher_before=teacher_before,A_teacher_after=teacher_after,A_teacher_predicted_delta=scale*sr_derivative,
      A_teacher_actual_delta=teacher_after-teacher_before,A_LR_derivative=lr_deriv,A_LR_train_L1_derivative=lr_l1_deriv,descent_alignment=alignment,
