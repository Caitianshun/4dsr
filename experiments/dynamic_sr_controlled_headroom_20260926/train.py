"""Fixed B topology: LR backward, controlled second backward, one Adam step.

The only scientific intervention is the second target: absent (Z), original
SwinIR (U), or privileged legal train HR (O). Migration occurs after step6000.
"""
import argparse
...
def run(a):
    ...
    assert start in ([0,6000,20] if a.smoke else [0,6000]) and start<a.stop
    ...
    if start==0:assert identity==read(a.start_identity)['identity']
    else:
    ...
    if a.method!='Z':
        ...
        assert bool(idx.get('privileged_train_hr',False))==(a.method=='O')
    ...
    else:assert a.target_index is None
    ...
    sources=[HERE/'train.py',ROOT/'experiments/dynamic_sr_motion_bound_20260923/motion_model.py',
    ...
    config=dict(method=a.method,privileged_train_hr=a.method=='O',manifest_sha256=sha256(a.manifest),schedule_sha256=sha256(a.schedule),
        common_start_identity=read(a.start_identity),restored_identity=identity,segment_start=start,segment_stop=a.stop,
        ...
        sr_weight=0. if a.method=='Z' else .1,renderer='unchanged ordinary_split Wu',topology='fixed 132972',
        second_backward='0.0 * prediction.float().mean()' if a.method=='Z' else '0.1 * mean(abs(prediction - target))',
        information_boundary='O alone reads privileged legal training HR. Z reads no SR/HR image. Shared B point selection previously used SR gradients.')
    ...
    with (out/'training.jsonl').open('w',buffering=1) as log:
        for step in range(start+1,a.stop+1):
            ...
            second=0.0*prediction.float().mean() if a.method=='Z' else .1*(prediction-target).abs().mean()
            assert bool(torch.isfinite(lr_loss+reg+second));second.backward()
            check=first or step in [6000,18000,40000] or step==a.stop
            ...
            if step in [6000,18000,40000] or step==a.stop:
                ...
                metadata=dict(scene=m['scene'],stage='controlled_headroom',branch='ordinary_split',method=a.method,privileged_train_hr=a.method=='O',
                    step=7200+step,intervention_step=step,manifest_sha=sha256(a.manifest),schedule_sha256=sha256(a.schedule),
                    extent=ck['metadata']['extent'],initial_identity=config['common_start_identity']['identity'],
                ...
                payload=dict(model=g.capture(),hidden=vars(model.h),optim=vars(model.o),metadata=metadata,rng=rng(),
                    samplers=dict(lr=lr_rng.getstate(),sr=sr_rng.getstate(),source_prefix_exposure={},additional_exposure=dict(exposure)),motion_refinement=refinement_state(model))
    ...
    write_json(out/'exposure.json',dict(lr=dict(lr_exposure),second_camera=dict(exposure),supervised_second_samples=0 if a.method=='Z' else a.stop,
...
def main():
    ...
    for k in ['manifest','resume','schedule','start-identity','lr-curve','out']:p.add_argument('--'+k,type=Path,required=True)
    p.add_argument('--target-index',type=Path);p.add_argument('--method',choices=['Z','U','O'],required=True);p.add_argument('--stop',type=int,required=True);p.add_argument('--smoke',action='store_true')
    a=p.parse_args();assert a.stop in ([20,40] if a.smoke else [6000,40000])
    try:run(a)
