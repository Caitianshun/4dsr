#!/usr/bin/env python3
"""CPU fixed native detail-supervision crops and a60-frame comparison video.
...
shared=module_from('detail_views_summary_helpers',Path(__file__).with_name('summarize.py'))
...
def export(args):
    ...
    for name,item in methods.items():
        ...
        if teacher_config is None:
            ...
            old_teachers = {(c,f):h for c,f,h in shared.teacher_identity(teacher_config)}
            for camera, frame, digest in shared.teacher_identity(config):
                if (camera,frame) in old_teachers:
                    require(old_teachers[camera,frame]==digest,'Shared training teacher identity differs')
        endpoints = ['test_6000'] if item['group']=='postprocess' else ['train_fixed_6000','test_6000']
    ...
    def images_for(camera,frame,teacher=False):
        ...
        for name,item in methods.items():
            if (name,endpoint) not in evaluations:
                require(item['group']=='postprocess' and endpoint=='train_fixed_6000','Missing model evaluation')
                continue
        ...
        if teacher:
            ...
            teacher_path=args.manifest.parent/teacher_config.get('prior_subdir','sr_swinir_x4')/camera/Path(obs['lr_path']).name
            image,digest=checked_image(teacher_path,matches[0]['sha256'])
            require(image.size==hr.size,'Teacher size mismatch')
            images['Frozen SwinIR (train)']=image
            sources.append({'role':'train_teacher','camera':camera,'frame':frame,'path':str(teacher_path),'sha256':digest})
        return images,obs
    ...
    result={'status':'completed_fixed_views','scene':manifest['scene'],'roi_protocol':str(args.roi_protocol),
            ...
            'source_images':sources,'labels':{n:i['label'] for n,i in methods.items()},'comparison_groups':{n:i['group'] for n,i in methods.items()},'source_sha256':sha(__file__),
            'precision':'Native PNG stills/crops and display-resized video; these quantized previews do not replace float-render metrics.',
            'information_boundary':'Post-SR is shown separately labelled only on held-out views and consumes model render, never actual LR. No model/teacher generation. HR for evaluation/display; cached teacher shown only at traincam02. Fixed ROIs precede prediction reads.',
    ...
    lines=[f'# {manifest["scene"]}：固定图像与60帧对照', '',
           '复用上一轮固定ROI与坐标，不随本轮方法结果更改；坐标未跟踪运动，名称仅描述参考帧内容。不同相机单独登记，不宣称精确物理对应。训练图额外显示已缓存教师，测试图不使用测试教师。', '',
    ...
    lines += ['B4后处理若列入，标签单列且需要额外SwinIR网络，不是F推理路径。视频为缩小展示，原生全图与168×168局部另存。量化展示不替代浮点指标；不存在由这些图自动确认的方法收益。','']
