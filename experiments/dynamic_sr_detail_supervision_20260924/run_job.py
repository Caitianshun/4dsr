#!/usr/bin/env python3
"""One registered U/W/F job: train/reuse, then immediately evaluate.
...
class Job(soft.Job):
    def __init__(self, args):
        ...
        self.inputs = {name: identity(getattr(args, name)) for name in
                       ('manifest', 'parent', 'selection', 'teacher_index', 'schedule', 'roi_protocol')}
        if args.calibration:
            self.inputs['calibration'] = identity(args.calibration)
        self.state.update(runner=identity(__file__), inherited_runner=identity(soft.__file__),
                          ...
                          fixed_sr_weight=.2 if args.method == 'W' else .1)
    ...
    def validate_training(self, folder):
        ...
        if args.method == 'F':
            expected['calibration_sha256'] = self.inputs['calibration']['sha256']
        for key, value in expected.items():
            if config.get(key) != value:
                raise RuntimeError(f'Training identity mismatch for {key}: {config.get(key)!r} != {value!r}')
        if 'sr_weight' in config and config['sr_weight'] != (.2 if args.method == 'W' else .1):
...
def main():
    ...
    for name in ('manifest', 'parent', 'selection', 'teacher-index', 'schedule', 'roi-protocol', 'out'):
    ...
    if args.method == 'F' and args.calibration is None:
        parser.error('F requires the frozen calibration JSON')
    for name in ('manifest', 'parent', 'selection', 'teacher_index', 'schedule', 'roi_protocol', 'calibration'):
