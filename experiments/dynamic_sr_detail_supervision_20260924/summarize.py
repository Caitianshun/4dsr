#!/usr/bin/env python3
"""Summarize U/W/F, coverage B4, historical methods and post-SR separately.
...
def load_methods(path):
    ...
    for name, item in methods.items():
        ...
        result[name] = {**item, 'checkpoint': checkpoint, 'train_dir': train_dir,
                        ...
                        'group': item.get('group', 'main' if item.get('role',name) in ['U','W','F'] else
...
def summarize(manifest_path, methods_path, out):
    ...
    main_methods = [n for n,i in methods.items() if i['role'] in ['U','W','F']]
    if main_methods:
        require({methods[n]['role'] for n in main_methods} == {'U','W','F'}, 'Main summary waits for all U/W/F')
    ...
    for name, item in methods.items():
        ...
        if item['role'] in ['U','W','F']:
            ...
            else: require(mc == main_control, 'U/W/F teacher pool or split initialization differs')
            require(set(config['teacher_cameras']) == set(manifest['splits']['train']), 'U/W/F must cover exactly the legal training cameras')
        ...
        for endpoint, location in item['evaluations'].items():
            ...
            expected_branch={'A':'joint','B':'ordinary_split','B4':'ordinary_split','C':'bound_split','S':'ordinary_split','U':'ordinary_split','W':'ordinary_split','F':'ordinary_split','B4_post':'ordinary_split','postprocess':'ordinary_split'}.get(item['role'])
            ...
            if item['role'] in ['U','W','F']:
                ...
                require(values.get('postprocess') is None, 'U/W/F must not use post-SR inference')
            ...
            elif item['role'] in ['U','W','F','B4','B4_post','postprocess']:
            ...
            draw_group = 'UWF' if item['role'] in ['U','W','F'] else name
            ...
            if item['role'] in ['U','W','F']:
                ...
                require(schedule is not None, f'{name}: fixed UWF schedule identity missing')
                schedule_key = 'UWF/schedule'
                if schedule_key in draws: require(schedule == draws[schedule_key], 'U/W/F schedule mismatch')
    ...
    if main_methods:
        base_cost = next(c for c in costs if c['method']==main_methods[0])
        for name in main_methods[1:]:
            current = next(c for c in costs if c['method']==name)
            for field in ['active_gaussians','stored_gaussians','trainable_parameters','child_parameters']:
                require(current[field] == base_cost[field], 'U/W/F inference capacity differs')
    deltas=[]
    index={(r['method'],r['split'],r['step'],r['region']):r for r in rows}
    pairs = [('F','U'),('F','W'),('U','W'),('U','B4'),('W','B4'),('F','B4'),('B4_post','B4')]
    ...
    lines=[f'# {manifest["scene"]}：教师覆盖与细节监督', '',
           '固定6000步；原四相机16张训练诊断、dev/test各60帧分列。U/W/F同教师池同采样，只按预注册权重/目标区分。B4的SR覆盖/流不同，历史A/C/S与后处理另表。', '',
    ...
    lines += ['', '训练硬件不同时不把耗时比归为算法开销；新增教师覆盖/缓存成本需另列，不能称相同信息预算。检查点字节含训练状态，不是纯推理权重体积。', '',
              'U/W/F主比较、覆盖比较及后处理具有不同解释范围；本脚本不自动触发扩展实验。单种子、已观察开发视角与60帧合成×4短窗不支持显著性/完整基准/真实纹理结论。', '']
