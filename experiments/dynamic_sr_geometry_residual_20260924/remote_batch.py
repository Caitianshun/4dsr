#!/usr/bin/env python3
"""Sequential G/L batch and event-driven remote dispatch/verified return.

run --spec JSON --out NEW_DIR executes one scene's exact G,L sequence.
The spec contains scene, common runner arguments (underscore keys), methods {G:{},L:{}}, nonempty file_identities and optional
...
def validate_spec(spec):
    common = spec['common']
    check(set(spec['methods']) == {'G', 'L'}, 'Exactly G/L; no additional branch')
    required = {'manifest', 'parent', 'selection', 'teacher_index', 'schedule', 'roi_protocol',
                'original_prior_cameras', 'train_gpu', 'eval_gpu', 'reference_config'}
    ...
    for method in ('G', 'L'):
        ...
        check(not options, 'No per-method tuning allowed')
    ...
    for key in ('manifest', 'parent', 'selection', 'teacher_index', 'schedule', 'roi_protocol', 'reference_config'):
...
def runner_command(common, method, extra, out):
    ...
    command = [sys.executable, str(HERE / 'run_job.py')]
...
def run(args):
    ...
    tick = time.monotonic()
    state = {'status': 'running_sequential_batch', 'scene': spec['scene'], 'order': ['G', 'L'],
    ...
    try:
        ...
        check(draws[0] and len(set(draws)) == 1, 'G/L realized draw streams differ or are absent')
...
def dispatch(args):
    ...
    try:
        module = args.remote_root.rstrip('/') + '/experiments/dynamic_sr_geometry_residual_20260924/remote_batch.py'
