"""G/L geometry residual: fixed U loss, sampler and split initialization."""
...
from geometry_model import (attach, geometry_state, synchronize_lr, residual_statistics,
                            render_model, capacity_summary)
from training_support import read, load_teacher_index, TeacherCache, initial_identity
...
def save(path, model, metadata, lr_rng, sr_rng, exposure):
    ...
    payload = dict(model=model.g.capture(), hidden=vars(model.h), optim=vars(model.o),
                   ...
                   geometry_residual=geometry_state(model))
...
def run(args, out):
    ...
    reference = read(args.reference_config)
    if identity != reference['initial_identity']:
        raise ValueError('Old model/optimizer initialization differs from reused U')
    for key, actual in [('manifest_sha256',manifest_hash),('parent_sha256',parent_hash),
                        ('selection_sha256',selection_hash),('schedule_sha256',sha256(args.schedule)),
                        ('teacher_index_sha256',sha256(args.teacher_index))]:
        if reference[key] != actual: raise ValueError(f'U input identity mismatch: {key}')
    if reference['gpu'] != torch.cuda.get_device_name() or reference['torch'] != str(torch.__version__):
        raise ValueError('G/L training hardware/environment differs from reused U')
    attach(model, args.method, m)
    initial_capacity = capacity_summary(model)
    if model.geometry_optimizer.state: raise AssertionError('Residual Adam must start empty')
    g.update_learning_rate(ck['metadata']['step'] + 1)
    first_lr = synchronize_lr(model)
    g.update_learning_rate(ck['metadata']['step'] + 6000)
    last_lr = synchronize_lr(model)
    # update_learning_rate changes no optimizer moments. Restore parent-step LR
    # until the first actual update, which uses offset + 1 as the old U loop.
    g.update_learning_rate(ck['metadata']['step'])
    synchronize_lr(model)
    geometry_protocol = dict(basis=model.geometry.basis.protocol(),
        residual_parameters=model.geometry.coeff.numel(), residual_optimizer_initial_step=0,
        source_group='xyz, identity checked against g._xyz', first_lr=first_lr,last_lr=last_lr,
        spatial_lr_scale=g.spatial_lr_scale, offset=ck['metadata']['step'], multiplier=1,
        adam={k:v for k,v in model.geometry_optimizer.param_groups[0].items() if k not in ['params']},
        sh='native rasterizer uses final corrected world means3D',
        source_time='unchanged manifest frame/300', residual_time='real camera image_name frame/118')
    source_files = [Path(__file__), MOTION / 'motion_model.py',
                    DETAIL / 'detail_loss.py', DETAIL / 'training_support.py',
                    Path(__file__).with_name('geometry_model.py'), Path(__file__).with_name('time_basis.py'),
    ...
    config = dict(**vars(args), manifest_sha256=manifest_hash, parent_sha256=parent_hash,
                  ...
                  schedule_sha256=sha256(args.schedule), calibration_sha256=None,
                  ...
                  inference='ordinary_split U plus eight-term child world center time residual before rasterization',
                  geometry_protocol=geometry_protocol, reference_config_sha256=sha256(args.reference_config), 
                  ...
                  photometric_loss='unchanged U: original LR mean L1 + 0.1 full RGB teacher mean L1 + original deformation regularizer',
    ...
    detail_metadata = dict(method=args.method, alpha=alpha,sr_weight=args.sr_weight,operator=OPERATOR,
                           ...
                           detail_loss_sha256=sha256(DETAIL/'detail_loss.py'))
    ...
    geometry_parameters = list(model.geometry.parameters())
    all_parameters = base_parameters + child_parameters + geometry_parameters
    ...
    with (out / "training.jsonl").open("w", buffering=1) as log:
        for step in range(1, args.steps + 1):
            ...
            coefficient_lr = synchronize_lr(model)
            ...
            teacher_total, sr_loss, detail_loss = teacher_loss(prediction,target,'U',0.,target_lr.shape[-2:])
            ...
            norms = ({"base": gradient_norm(base_parameters), "children": gradient_norm(child_parameters), "world_center_coeff": gradient_norm(geometry_parameters)}
                     if audit else None)
            g.optimizer.step()
            ...
            if step == 1 or step % 100 == 0 or step in milestones:
                row = dict(step=step, coefficient_lr=coefficient_lr, lr_l1=float(lr_loss), teacher_l1=float(sr_loss),
            ...
            if step in milestones:
                write_json(out/f'residual_statistics_{step}.json', residual_statistics(model))
                ...
                metadata = dict(scene=m["scene"], stage="geometry_residual", branch=args.branch, method=args.method, detail_supervision=detail_metadata,
