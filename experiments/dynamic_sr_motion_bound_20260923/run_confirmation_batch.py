"""Two new refinement arms; reuse this confirmation run's completed A training and reevaluate all three uniformly."""
import argparse
import csv
...
def main():
    ...
    state = dict(status='running', pid=os.getpid(), started_utc=datetime.now(timezone.utc).isoformat(),
                 ...
                 note='A newly trained for this confirmation sequence and reused for uniform evaluation; B/C train on the same GPU as A; all nine endpoints evaluate on the configured eval GPU; no remote jobs.')
    ...
    with (out / 'summary.log').open('w') as log:
        rc = subprocess.run([sys.executable, str(scripts / 'summarize_confirmation.py'), '--root', str(out)],
