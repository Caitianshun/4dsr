...
def markdown(summary):
    lines = ['# MeetRoom 完整场景运动绑定细分：固定端点汇总', '',
             'A 是上一轮 Joint 的既有检查点；本轮仅统一重评，不重训。B 是普通空间细分，C 是共享父运动的绑定细分。保留完整人物与复杂背景，不作人体抠图；所有主指标使用本轮统一脚本的浮点渲染。', '',
    ...
    lines += ['', '预算检查只约束参与渲染的活跃高斯增加不超过 20%。存储高斯、父控制量、可训练标量和固定缓冲另列，不能因此声称总容量或训练预算相同。A 的训练成本引用历史完整训练记录，本轮仅有重新评价成本。', '',
              ...
              '图像均为固定 test cam00/frame40。局部坐标继承 9 月 20 日审计，原先依据 HR 高频能量选取，未按本轮收益选择，仅作固定展示。局部是原生分辨率 8-bit PNG 裁剪，全图仅作显示缩小；两者不能替代浮点主指标。', '',
              '一个已使用的开发场景、一个采样种子、60 帧短窗。连续帧不独立，逐帧胜数和帧标准差均不构成显著性检验。dev/1200、dev/6000、test/6000 全部保留，不挑最佳端点。本汇总不自动宣布方法有效。', '',
...
def main(args):
    ...
    for branch in BRANCHES:
        ...
        costs.append({'branch': branch, **branch_capacity, 'capacity': branch_capacity,
                      ...
                      'historical_training_reused': branch == 'joint',
    ...
    result = {'status': 'descriptive_summary', 'root': str(root), 'output': str(out),
              ...
              'information_boundary': 'Completed-result CPU aggregation only. Full-scene RGB retained. GT masks/flows and held-out observed LR are evaluation-only. A reuses historical checkpoints but all evaluations use the same float-render protocol.',
              'limitations': ['One already-used development scene, one explicit sampling seed, nonstandard 60-frame short-window pilot.',
                              ...
                              'Historical fixed ROI panels use saved uint8 predictions; main metrics use float renders.'],
