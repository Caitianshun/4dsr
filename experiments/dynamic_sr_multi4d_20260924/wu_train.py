"""U40/W40 exact saved-prefix continuation; native schedules are not stretched."""
...
from motion_model import load_model, OLD, capacity_summary, make_model, refinement_state, render_model
...
def run(args, out):
    ...
    prefix = read(Path(args.resume).parent / 'config.json')
    assert prefix['method'] == args.method and prefix['steps'] == 6000
    assert prefix['parent_sha256'] == parent_hash and prefix['manifest_sha256'] == manifest_hash
    for source in prefix['sources']:
        # Relocation does not change source identity; resolve upstream separately.
        name = Path(source['path'])
        if '/4dgs/' in str(name): name = UPSTREAM / str(name).split('/4dgs/', 1)[1]
        else: name = ROOT / str(name).split('/4dsr/', 1)[1]
        assert sha256(name) == source['sha256'], str(name)
    model = load_model(args.resume, m)
    g, h, o, resumed = model.g, model.h, model.o, model.checkpoint
    assert resumed['metadata']['intervention_step'] == 6000
    assert resumed['metadata']['method'] == args.method
    assert resumed['metadata']['manifest_sha'] == manifest_hash
    assert_state_equal(resumed['model'][12], g.optimizer.state_dict())
    assert_state_equal(resumed['motion_refinement']['child_optimizer'], model.child_optimizer.state_dict())
    ck = dict(metadata=prefix['parent_metadata'], rng=resumed['rng'])
    selection_hash = sha256(args.selection)
    assert selection_hash == prefix['selection_sha256']
    assert initial_identity(model)['model_and_optimizer_sha256'] == __import__('training_support').digest_state(
        (resumed['model'],resumed['motion_refinement']['children'],resumed['motion_refinement']['child_optimizer']))
    ...
    assert schedule['manifest_sha256'] == manifest_hash and schedule['steps'] == 40000 and schedule['seed'] == args.seed
    ...
    if args.method == 'F':
        ...
        expected = dict(manifest_sha256=manifest_hash,parent_sha256=parent_hash,selection_sha256=selection_hash,
                        ...
                        detail_loss_sha256=sha256(ROOT / 'experiments/dynamic_sr_detail_supervision_20260924/detail_loss.py'))
    ...
    identity = prefix['initial_identity']
    ...
    old_schedule = read(args.prefix_schedule)
    assert old_schedule['rows'] == schedule['rows'][:6000]
    assert sha256(args.prefix_schedule) == prefix['schedule_sha256']
    assert sha256(args.teacher_index) == prefix['teacher_index_sha256']
    source_files = [Path(__file__), MOTION / 'motion_model.py',
                    ROOT / 'experiments/dynamic_sr_detail_supervision_20260924/detail_loss.py',ROOT / 'experiments/dynamic_sr_detail_supervision_20260924/training_support.py',
    ...
    # Replay CPU draw hashing only, without model work or changing global RNG.
    for li, si, old_si in schedule['rows'][:6000]:
        assert lr_rng.randrange(len(records)) == li and sr_rng.choice(old_ids) == old_si
    ...
    assert lr_rng.getstate() == resumed['samplers']['lr']
    assert sr_rng.getstate() == resumed['samplers']['sr']
    assert dict(exposure) == resumed['samplers']['additional_exposure']
    for key, value in [('draw_sha256',draw.hexdigest()),('lr_draw_sha256',lr_draw.hexdigest()),('sr_frame_sha256',sr_times.hexdigest())]:
        assert resumed['metadata'][key] == value
    # Freeze all 40000 actual LR values before any resumed update.
    curve = []
    for step in range(1,40001):
        g.update_learning_rate(7200+step)
        curve.append([step, {q['name']:q['lr'] for q in g.optimizer.param_groups},
                      {q['name']:q['lr'] for q in model.child_optimizer.param_groups}])
    assert all(math.isfinite(v) and v>0 for _,a,b in curve for v in list(a.values())+list(b.values()))
    write_json(out/'learning_rates.json',dict(rule='Unchanged native exponential clipped at original max_steps; then fixed positive final LR',rows=curve))
    write_json(out/'prefix_audit.json',dict(status='passed',checkpoint_sha256=sha256(args.resume),
        restored_model_optimizer=initial_identity(model),completed_prefix_updates=6000,
        exact_schedule_prefix=True,optimizer_states_equal=True,rng_available=True,
        prefix_gpu=prefix['gpu'],resume_gpu=torch.cuda.get_device_name()))
    g.update_learning_rate(13200)
    audit_seconds = 0.0
    ...
    with (out / "training.jsonl").open("w", buffering=1) as log:
        for step in range(6001, args.steps + 1):
            tick = time.monotonic()
            ...
            audit = step == 6001 or step in milestones
            target = cache.get(si)
            ...
            exposure[f'{records[si]["camera_id"]}/{records[si]["frame_index"]}'] += 1
            if step == 6001 or step % 100 == 0 or step in milestones:
    ...
    complete = dict(**metadata, status="completed", parameter_updates=args.steps-6000, total_sr_updates=args.steps, reused_prefix_updates=6000,
...
def main():
    ...
    parser.add_argument("--method", choices=['U','W'], required=True)
    parser.add_argument("--teacher-index", required=True)
    parser.add_argument("--schedule", required=True)
    parser.add_argument("--calibration")
    parser.add_argument("--steps", type=int, default=40000)
    parser.add_argument("--milestones", default="18000,40000")
    ...
    parser.add_argument('--prefix-schedule', required=True)
    args = parser.parse_args()
    if args.smoke:
        args.steps, args.milestones = 6020, "6020"
    if args.steps != 40000 and not args.smoke:
        parser.error('Formal protocol is exactly40000 total SR updates')
