"""Position-path and SR-only appearance routing from complete shared checkpoints.
...
from gradient_policy import render_model, policy_info, backward_prior, position_locked
...
def run(a):
    ...
    assert start in ([6000,6002] if a.smoke else [6000,12000,17550]) and start<a.stop
    ...
    if not a.smoke: assert read(a.protocol.parent/'engineering_v1/complete.json')['status'].startswith('passed')
    ...
    if start==6000:
        ...
        assert ck['metadata']['schedule_sha256']==sha256(a.schedule)
        assert ck['metadata']['method']==a.method or (start==17550 and ck['metadata']['method']=='C_joint' and a.method in ['P_late','A_late'])
    ...
    policy=policy_info(model,a.method)
    topology_reference=topology(model)
    paths={};inputs=[]
    if True:
    ...
    sources=[HERE/'train.py',HERE/'gradient_policy.py',HERE/'shared.py',ROOT/'experiments/dynamic_sr_motion_bound_20260923/motion_model.py',
    ...
    config=dict(method=a.method,privileged_train_hr=a.method=='O',manifest_sha256=sha256(a.manifest),schedule_sha256=sha256(a.schedule),
        ...
        sr_weight=.1,renderer='ordinary_split Wu; identical forward, optional final xyz detach on SR only',topology='fixed 132972',
    ...
    write_json(out/'routing_audit.json',dict(policy=policy,checks=[]))
    ...
    with (out/'training.jsonl').open('w',buffering=1) as log:
        for step in range(start+1,a.stop+1):
            ...
            prediction=render_model(model,cameras[si],detach_prior_position=position_locked(a.method,step,a.smoke))['render']
            second=.1*(prediction-target).abs().mean()
            assert bool(torch.isfinite(lr_loss+reg+second));backward_prior(second,model,a.method,step,a.smoke)
            check=first or step in [12000,17550,18000] or step==a.stop
            ...
            if first:
                ...
                if a.method=='A_late' and (a.smoke or step>17550):
                    assert all(d is None or d==0 for (n,p),d in zip(named,delta) if n not in policy['sr_appearance_names'])
            ...
            if check:
                assert topology(model)==topology_reference
                audit=read(out/'routing_audit.json');audit['checks'].append(dict(step=step,topology_unchanged=True,position_detached=position_locked(a.method,step,a.smoke),lr_full_path=True,finite=True));write_json(out/'routing_audit.json',audit)
            ...
            if step in [12000,17550,18000] or step==a.stop:
                ...
                metadata=dict(scene=m['scene'],stage='prior_guidance',branch='ordinary_split',method=a.method,privileged_train_hr=a.method=='O',
                ...
                payload=dict(model=g.capture(),hidden=vars(model.h),optim=vars(model.o),metadata=metadata,rng=rng(),
                    samplers=dict(lr=lr_rng.getstate(),sr=sr_rng.getstate(),source_prefix_exposure={},additional_exposure=dict(exposure)),motion_refinement=refinement_state(model),prior_guidance=dict(policy=policy,topology_reference=topology_reference))
                ...
                if a.smoke:
                    ...
                    assert policy_info(reloaded,a.method)==policy
                    assert topology(reloaded)==topology_reference
...
def main():
    ...
    p.add_argument('--target-index',type=Path,required=True);p.add_argument('--method',choices=['C_joint','P_early','P_late','A_late'],required=True);p.add_argument('--stop',type=int,required=True);p.add_argument('--smoke',action='store_true')
    a=p.parse_args();assert a.stop in ([6002,6004] if a.smoke else [12000,17550,18000])
    try:run(a)
