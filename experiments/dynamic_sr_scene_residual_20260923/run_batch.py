"""One training GPU, one evaluation GPU; child exit directly enqueues evaluation."""
import argparse
import csv
...
def main():
    ...
    state = dict(status='running', pid=os.getpid(), started_utc=datetime.now(timezone.utc).isoformat(),
                 ...
                 note='All training on one GPU; all evaluation on another GPU; no remote jobs.')
    ...
    try:
        for branch in state['branches']:
            child(f'{branch}_train', [sys.executable, str(scripts / 'train.py'), '--manifest', args.manifest,
                  '--checkpoint', args.checkpoint, '--out', str(out / branch), '--branch', branch,
