...
def foreground_l1(prediction: torch.Tensor, target: torch.Tensor,
                  mask: torch.Tensor) -> torch.Tensor:
    """0.8 foreground + 0.2 background mean RGB L1, with soft LR-derived masks.

    Each region has its own pixel denominator. An absent region contributes
    zero; its fixed weight is not transferred to the other region.
    """
    if prediction.shape != target.shape or prediction.ndim != 3:
        raise ValueError("Prediction and target must have equal CHW shapes")
    if mask.ndim == 3 and mask.shape[0] == 1:
        mask = mask[0]
    if tuple(mask.shape) != tuple(prediction.shape[-2:]):
        raise ValueError("Mask and image spatial dimensions differ")
    error = (prediction - target).abs().mean(0)
    return (0.8 * (error * mask).sum() / mask.sum().clamp_min(1e-12)
            + 0.2 * (error * (1 - mask)).sum() / (1 - mask).sum().clamp_min(1e-12))
